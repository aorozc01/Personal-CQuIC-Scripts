function [std_FC_spectrum,std_PWC_spectrum,Avg_FC_Fidelity,Avg_PWC_Fidelity,...
    freq_FC,FC_power_spectrum,freq_PWC,PWC_power_spectrum,...
    FC_dt,PWC_dt,FC_phase,PWC_phase,optimal_coeff,...
   optimal_PWC_coeff,target_state,t_final,steps_FC,...
    initial_state,psi_f_FC,psi_f_PWC] = ...
    Control_Optimization(spin,beta,Omega,w0,NumAvgs,plots)
% [std_FC_spectrum,std_PWC_spectrum,Avg_FC_Fidelity,Avg_PWC_Fidelity,...
%     freq_FC,FC_power_spectrum,freq_PWC,PWC_power_spectrum,PWC_FC,T_PWC_FC,PWC_PWC,...
%     T_PWC_PWC,FC_dt,PWC_dt,FC_FT,PWC_FT,FC_phase,PWC_phase,optimal_coeff,...
%     PWC_phiF_fft,PWC_phiG_fft,df,optimal_PWC_coeff,target_state,t_final,steps_FC,...
%     initial_state,psi_f_FC,psi_f_PWC] = ...
%     Control_Optimization(spin,beta,Omega,w0,NumAvgs,plots)
...
for jj = 1:NumAvgs
    ...
    [FC_Fidelity(jj),FC_Gradient,FC_phase,FC_dt,psi_f_FC] = FourierConstrainedPhase...
    ...
    [PWC_Fidelity(jj),PWC_Gradient,PWC_phase,PWC_dt,psi_f_PWC] = PWC_Phase...
...
%---Find Variance of the Power Spectra of FC and PWC optimal phase
[std_FC_spectrum,std_PWC_spectrum]=spectrum_std(freq_FC,FC_power_spectrum,...
    freq_PWC,PWC_power_spectrum);

if plots ==1
hold on;grid on;subplot(1,2,2)
plot(std_PWC_spectrum,linspace(0,1,100),'.k',-std_PWC_spectrum,linspace(0,1,100),'.k')
plot(std_FC_spectrum,linspace(0,1,100),'.m',-std_FC_spectrum,linspace(0,1,100),'.m')
end
    %}
end