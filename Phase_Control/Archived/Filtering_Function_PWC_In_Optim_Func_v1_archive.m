...
for kk = 1:length(PB_butter_PWC)
    ...
    PWC_butter = filtfilt(B,A,PWC_PWC); % output is filtered function
    psif_filt = Weak_Dressed_H(PWC_butter,initial_state,target_state,...
        spin,t_final,1,1);
    d_PWC_butter(kk) = abs(psif_filt'*psif)^2;
    if d_PWC_butter(kk) >= 0.99
            wmax_PWC = PB_butter_PWC(kk);
        break
    end
end
