...
PB_butter = (1:0.5:30)*(w0/(2*pi))/(fs/2); % normalized Passband
d_FC_butter = zeros(length(PB),1);
psif = Weak_Dressed_H(PWC_FC,initial_state,target_state,spin,t_final,1,1);
Fid_FC = d_PWC_butter;
for kk = 1:length(PB_butter)
    [B,A] = butter(5,PB_butter(36)); % by default butter produces a lowpass filter
...
%     legend('Filtered','Not-Filtered')
    ...
    d_FC_butter(kk) = abs(psif_filt'*psif);
%     Fid_FC(kk) = abs(psif_filt'*target_state)^2;
end
figure;plot(PB_butter*(2*pi*fs/2),d_FC_butter,'*r');title('Fourier Constrained Phase')
