function [Fidelity,Gradient,phase,dt,psi_f] = PWC_Phase(const_phases,psi,psit,spin,t_final,beta,Omega)
...
for jj = 1:steps_length
    ...
    H_tot = beta*(Jz^2) + Omega*(cos(phase(jj))*Jx + sin(phase(jj))*Jy);
    ...
    dH_dphi = Omega*(-Jx*sin(phase(jj))+ Jy*cos(phase(jj)));
